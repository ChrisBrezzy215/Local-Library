function getTotalBooksCount(books) {
  // Use the .length property to get the number of book objects in the array
  return books.length;
}

function getTotalAccountsCount(accounts) {
  // Use the .length property to get the number of account objects in the array
  return accounts.length;
}

function getBooksBorrowedCount(books) {
  // Use reduce to count the number of books currently borrowed
  return books.reduce((count, book) => {
    // Check if the first transaction indicates the book is currently borrowed
    const isBorrowed = book.borrows[0].returned === false;
    
    // Increment the count if the book is currently borrowed
